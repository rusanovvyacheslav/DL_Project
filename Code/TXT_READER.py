...
PATH_LABELS = "../Annotated Files/Lables"
...
def READER(XL=336, YL=192):
    ...
    :return: OUTPUT_SAVE: The tensor storing all of the outputs (th.tensor).
             Valid_idx: A list containing the the valid scenes file name (list).
    """

    # Finding the valid scene values:
    Valid_idx = []  # The valid images
    N_Valid = 0  # Total amount of valid images (will increase)
    ...
    for N in range(len(Names)):  # from 0 to N_Img
        ...
        if match == None: # If it isn't a scene
            ...
            N_Valid += 1  # Total amount of valid images
    ...
    for N in range(len(Valid_idx)): # For the valid files:
        ...
        fullname = os.path.join(PATH_LABELS, filename)
        f = open(fullname, 'r')  # Opens the file (now guaranteed to work)
        for line in f:
            line = line.strip()
            result = line.split(" ")
